const template741v2Fn = (row, template) => {
  ...
  const loan_current_due_date =
    row.loan_current_due_date || 'no loan current due date';
  const loan_current_balance =
    row.loan_current_balance || 'no loan current balance';
...
module.exports = template741v2Fn;
